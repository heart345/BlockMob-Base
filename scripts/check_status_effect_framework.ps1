...
Assert-Contains $status 'status\.Effects\.poison[\s\S]*undeadImmune\s*=\s*true' "poison should declare undead immunity through effect metadata."
Assert-Contains $status 'effect\.undeadImmune\s+and\s+target\.IsUndead' "status.Apply should centrally reject effects tagged as undead-immune."
...
Assert-Contains $zombie 'ENT\.IsUndead\s*=\s*true' "zombie-family base should be tagged undead for status-effect immunity."
Assert-Contains $skeleton 'ENT\.IsUndead\s*=\s*true' "skeleton-family base should be tagged undead for status-effect immunity."
