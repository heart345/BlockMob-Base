...
Assert-Contains $spider 'function ENT:HasBMBSpiderClimbVerticalClearance\(pos,\s*normal\)' "Spider climb should reject low-ceiling crawl spaces before SetPos takes over."
Assert-Contains $spider 'self:GetBMBSpiderClimbMinVerticalClearance\(\)' "Spider climb vertical clearance should require real upward space, not just a side wall hit."
Assert-Contains $spider 'skip climb start: low ceiling' "Spider climb should log when a one-block/low-ceiling space is handed back to ground movement."
Assert-Contains $spider 'function ENT:ClampBMBSpiderClimbPosition\(pos\)' "Spider climb SetPos should clamp upward movement so it cannot push below the starting floor."
Assert-Contains $spider 'BMBSpiderClimbFloorZ\s*=\s*startPos\.z' "Spider climb should remember the starting floor before SetPos movement begins."
Assert-Contains $spider 'BMBSpiderClimbFloorZ\s*=\s*nil' "Spider climb finish should clear the floor clamp state."
...
Assert-Contains $spider 'HasBMBSpiderClimbVerticalClearance\(startPinned,\s*normal\)' "Spider climb startup should require vertical clearance at the pinned wall position."
Assert-Contains $spider 'pinned\s*=\s*self:ClampBMBSpiderClimbPosition\(pinned\)' "Spider climb should clamp each pinned SetPos target above the starting floor."
