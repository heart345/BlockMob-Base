...
Assert-Contains $spider 'function ENT:IsBMBSpiderClimbBlockingEntity\(ent\)' "Spider climb should detect players/NPCs/mobs before SetPos can tunnel through them."
Assert-Contains $spider 'ents\.FindInBox\(mins,\s*maxs\)' "Spider climb should scan the target hull volume for blocking entities."
Assert-Contains $spider 'function ENT:GetBMBSpiderClimbEntityBlock\(fromPos,\s*toPos\)' "Spider climb should expose an entity blocker check for SetPos movement."
Assert-Contains $spider 'GetBMBSpiderClimbEntityBlock\(fromPos,\s*toPos\)(?s).*if entityTrace then return false,\s*entityTrace end' "Spider climb movement hull checks should reject player/mob blockers before SetPos."
Assert-Contains $spider 'GetBMBSpiderClimbBlockingEntity\(pos\)' "Spider mantle landing clearance should reject player/mob blockers."
