...
Assert-Contains $base "HasBMBDamageType\(damageInfo,\s*DMG_POISON\)" "poison DoT damage should keep damage/flash but must not trigger BMB knockback"
...
Assert-Contains $base "function\s+ENT:ArmBMBKnockbackHopSuppress" "post-knockback hop suppression should have a shared arming helper"
Assert-Contains $base "function\s+ENT:MaintainBMBKnockbackHopSuppressFallback" "post-knockback hop suppression should have a Think fallback when OnLandOnGround is missed"
Assert-Contains $base "MaintainBMBKnockbackHopSuppressFallback\(\)" "base Think grounded maintenance should arm hop suppression if the land callback is missed"
Assert-Contains $base "BMBKnockbackSuppressHopOnLand\s*=\s*self\.BMBKnockbackSuppressHopOnLand\s+or" "airborne follow-up hits must not clear a pending post-knockback hop suppression"
Assert-Contains $base "BlockHopPostKnockbackSuppressDuration\s*=\s*1\.25" "post-knockback hop suppression should last long enough to cover the first grounded chase tick"
Assert-Contains $base "BlockHopPostKnockbackSuppressMoveDistanceScale\s*=\s*1\.25" "post-knockback hop suppression should not clear after only a short chase step"
Assert-Contains $base "BMBKnockbackSuppressHopSawAir" "fallback hop suppression should wait until the knockback launch has actually left the ground"
Assert-Contains $base "not self:IsBMBOnGround\(\)[\s\S]*BMBKnockbackSuppressHopSawAir\s*=\s*true" "fallback should mark airborne state before arming post-knockback hop suppression"
Assert-Contains $base "not self\.BMBKnockbackSuppressHopSawAir[\s\S]*return false" "fallback should not arm the suppression window before seeing the launched mob leave the ground"
Assert-Contains $base "KnockbackUseJump" "grounded knockback jump-state opening should be configurable per mob"
Assert-Contains $base "KnockbackUseJump\s*~=\s*false[\s\S]*self\.loco:Jump\(\)" "base knockback should only open locomotion jump state for mobs that opt in"
Assert-Contains $base "DeathKnockbackDuration\s*=\s*0\.22" "lethal hits should keep a short MC-like knockback window during the death animation"
Assert-Contains $base "function\s+ENT:StartBMBDeathKnockback" "death should preserve a fatal-hit shove instead of freezing immediately"
Assert-Contains $base "function\s+ENT:RunBMBDeathKnockback" "dead mobs should run one short knockback window before the frozen death pose takes over"
Assert-Contains $base "self:StartBMBDeathKnockback\(damageInfo\)" "BeginBMBDeath should start fatal knockback after entering dead state"
Assert-Contains $base "self:RunBMBDeathKnockback\(\)" "base Think should allow death knockback before zeroing dead loco velocity"
...
Assert-FunctionNotContains $base "Draw" "BMBHurtFlashDuration|math\.Clamp\(\(flashUntil - CurTime\(\)\)|local\s+amount" "hurt flash should be instant constant red for the whole invulnerability window, not a fade curve"
...
Assert-Contains $sheep "function\s+ENT:GetBMBSheepFullBodyFlashAmount" "sheep should detect the shared hurt/death flash window for its full-body material overlay"
Assert-Contains $sheep "function\s+ENT:DrawBMBSheepBaseWithoutFlash" "sheep flash should draw the tinted sheep normally before the full-body flash overlay"
Assert-Contains $sheep "function\s+ENT:DrawBMBSheepFullBodyFlash" "sheep needs one full-body flash overlay because base color modulation does not cover all sheep materials"
Assert-Contains $sheep "self:DrawBMBSheepBaseWithoutFlash\(\)[\s\S]*self:DrawBMBSheepFullBodyFlash\(flashAmount\)" "sheep flash should be normal draw plus one overlay, not base red modulation plus overlay"
...
Assert-Contains $zombie "KnockbackUseJump\s*=\s*true" "zombie hurt knockback should restore MC-style grounded lift now that hop launch is grounded-gated"
Assert-Contains $zombie "KnockbackVerticalMaxSpeed\s*=\s*240" "zombie hurt knockback should use the shared vertical lift cap"
Assert-Contains $skeleton "KnockbackUseJump\s*=\s*true" "skeleton-family hurt knockback should restore MC-style grounded lift now that hop launch is grounded-gated"
Assert-Contains $skeleton "KnockbackVerticalMaxSpeed\s*=\s*240" "skeleton-family hurt knockback should use the shared vertical lift cap"
