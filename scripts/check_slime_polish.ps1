...
Assert-Contains $slime 'function ENT:GetBMBSlimeDeathTipLift\(tip,\s*horizontal\)' "Slime death tip should lift the render origin so it does not sink into the floor."
Assert-Contains $slime 'ENT\.DeathTipDegrees\s*=\s*90' "Slime death should visibly tip sideways before cleanup."
Assert-Contains $slime 'SetRenderAngles\(deathAngles\)' "Slime death draw should apply a temporary render-angle side tip."
Assert-Contains $slime 'RotateAroundAxis\(deathAngles:Right\(\),\s*tip\)' "Slime death should tip around a horizontal axis that is visible on the cube model."
Assert-Contains $slime 'SetRenderOrigin\(self:GetPos\(\) \+ Vector\(0,\s*0,\s*self:GetBMBSlimeDeathTipLift\(tip,\s*horizontal\)\)\)' "Slime death tip should keep the rendered model above the ground."
Assert-Contains $slime 'SetRenderAngles\(oldRenderAngles or self:GetAngles\(\)\)' "Slime draw should restore render angles after drawing."
