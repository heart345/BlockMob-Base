...
Assert-Contains $base "bmb_safety_ceiling_clearance" "Direct safety should expose ceiling clearance so low one-cell passages can be tuned without lowering ground probes"
Assert-Contains $base "local wallTopZ = math\.min\(" "Direct safety wall probe should clamp hull top below the mob's head instead of probing into the ceiling cell"
Assert-Contains $base "maxs = Vector\(self\.CollisionMaxs\.x \* hullScale,\s*self\.CollisionMaxs\.y \* hullScale,\s*wallTopZ\)" "Direct safety wall probe should use the clamped wallTopZ for its hull maxs"
