...
- [x] Hostile `chase_repath` cliff retry memory: A*-failure direct pressure now shares direct cliff memory, uses 25s fallback expiry plus conservative retry thresholds (3s / 6 mob cells / 2 target cells, convar tunable), and gives up back to target scan/wander if the blocked line remains impossible.
