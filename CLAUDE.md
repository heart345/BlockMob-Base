...
- 寻路是自写 A*，跑在方块网格上。**不用 navmesh**（盖不住动态体素）。real 方块世界启用 3D 邻接：同层 `walk`、+1 格 `hop`、向下 ≤3 格 `drop`；mock 仍是平面世界，避免调试地面被 z 轴语义污染。
...
- **跳一格用固定弹道 BlockHop**：顶点 = 1.25 格 = 45u，起跳瞬间 `loco:SetVelocity` 只加竖直分量 `vz = √(2 × loco:GetGravity() × 45)`、保留水平速度；滞空期间 `Approach` 无效，水平速度向目标方向弱 Lerp（系数 0.05~0.1），落地恢复正常跟随。hop 边由 A\* 在"+1 高度差且上方净空"时标记，跟随层消费到 hop 边、2D 距离够近且 `IsOnGround()` 时触发一次。`StepHeight` 保持 <36——不要靠 step 把 mob 滑上方块，视觉不是 MC 的跳跃感。`loco:SetVelocity` 不在"禁止手动驱动"之列（loco 层接口，不打断插值）。
