...
- **跳一格用 BlockHop**：优先用 `loco:JumpAcrossGap(landingGoal, landingForward)` 跳到上层格脚底落点（目标 foot cell 中心 z - 半格），并把 `SetJumpHeight` 抬到至少 58u；这是 NextBot 原生"跳到指定落点"原子操作，避免 `Jump()` 与同 tick `SetVelocity` 互相覆盖。原生 hop 期间只允许转向/刷新 watchdog，不能再 `SetVelocity` 弱改水平速度。老引擎没有 `JumpAcrossGap` 时才 fallback 到 `loco:Jump()` + `SetVelocity` 固定弹道。落地却没站上目标格 = 这跳失败，以 `OnLandOnGround` 回调时刻为基准短延时（必须 < watchdog grace）后复位重跳，连续失败数次按路径失败交还行为层。`StepHeight` 保持 <36——不要靠 step 把 mob 滑上方块，视觉不是 MC 的跳跃感。`loco:SetVelocity`/`loco:Jump`/`loco:JumpAcrossGap` 不在"禁止手动驱动"之列（loco 层接口，不打断插值）。
