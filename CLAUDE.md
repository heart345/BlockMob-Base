...
- **Flee 按 MC PanicGoal 实现**：受击触发，在半径约 5 格（垂直 ±4）内采样随机可达点——与攻击者方向**无关**；采样顺序是先用 `IBlockWorld` 枚举可站立格、再从中随机抽、最后 A\* 验证，限制每次尝试次数（约 8~10），全部失败则本次不触发 flee（原地，符合原版）。到点后仍在惊慌窗内则再选下一个点；速度默认参考 panic 倍率，但可按怪物和模型手感覆盖（当前 sheep `RunSpeed=100`、`FleeKeepFullSpeed=true`，恐慌窗口 `3.5-5.0s`）。
...
- **远程基础切片 = SeekTarget + RangedAttack + 弹丸 SENT + 怪物薄状态机**（骷髅）。`RangedAttack`（`sv_behaviors.lua`）解耦"我想站哪"和"我何时射"两件事：`Update(mob,target)` 每 tick 跑 `UpdateSightMemory`（`mob.BMBSeeTime` 秒累加，trace 眼→眼 LOS）→ `ResolveMovement`（远/没稳定看见 → 阻塞 `Chase.Run` 接近；够近且稳定看见 → aim 停下，**M2 才接 `UpdateStrafe` 风筝**）→ aim 时 `UpdateDrawFire`（满弓时长 + 攻击间隔冷却 + 丢失取消）→ `Fire`（MC 抛物线补偿 `d.z += horiz*0.2`，**Z-up 别照抄 MC Y-up 轴**，spawn 弹丸）。协程整合：chase 阻塞、aim 一 tick，由 `RunBehaviour` 的 `coroutine.yield/wait` 提供逐 tick 节奏。**弹丸（`bmb_arrow`）纯 GMod trace**：手动积分重力 + 沿位移 `TraceLine`，命中世界移除、命中生物 `TakeDamageInfo`；不碰体素系统（远程怪零跨系统依赖）。远程怪实体只放参数（range/cooldown/箭速伤害散布/索敌）、声音、目标类型；不要把拉弓/弹道逻辑写回单个怪。strafe（绕目标横移）是 aim 内生的移动原语，直接给 loco 速度但**必须**走 `Chase.IsSteerTargetSafe`/`IsMovementTargetSafe` 探边缘墙、身体持续 `FaceTarget`。
- **`Flee.Run(mob, threat)` 可选威胁参数**：不传 threat = MC PanicGoal（随机近点，与方向无关，羊/僵尸用）；传 threat（如骷髅逃狼）= AvoidEntityGoal 语义，`pickPanicDestination` 只收比当前离 threat 更远的候选点。逃威胁优先级高于攻击（MC priority 数字小者先占移动通道），分支先 return、不清 TargetEntity。
- **Look 类模块（如 LookAtPlayer）是非独占的**，与移动类模块并行：移动时身体朝向归移动管，look 只控制头部；静止时头先转（骨骼操纵，clamp yaw ±75°/pitch ±40°），头到极限才由 `loco:FaceTowards` 带动身体跟转。状态机调度需支持"一个移动模块 + 一个注视模块"同时活跃。敌对/远程怪用 `GetBMBForcedLookTarget` 返回 `TargetEntity` 让头追踪期间锁死目标（不受瞥视距离/概率限制）。
...
- 速度分两层：`BMBDesiredSpeed` 是当前给 `loco` 的命令速度，可以因 `path_corner`、drop、局部安全控制短暂变化；`BMBActivitySpeed` 是行为/动画意图速度，用来决定 idle/walk/run/jump。Flee/Panic 这类“正在跑”的行为必须保持 `BMBActivitySpeed=RunSpeed`，并用 `minPathSpeed` 把过弯临时降速夹在 run 阈值上方，避免套皮后跑/走动作来回抖；若某个 mob 需要 HUD 目标速度也不降（sheep 当前需求），用 `FleeKeepFullSpeed=true` 让 Flee path corner 仍保持满 `RunSpeed`。
...
- 动画/activity 选择要状态驱动：Think/落地时根据 held、空中、落地速度选择 idle/walk/run/jump，不能只在事件里 StartActivity 后不收回。默认旧模型仍可走 `StartActivity`；正式 MC 模型优先用 `AnimationSequences` 做逻辑动作到模型 sequence 别名的映射，例如 `{ idle="idle", walk="walk", run="walk", attack="attack", hurt="hurt", death="death" }`。转换器导出的 `$sequence` 名必须按 entity.json 动画别名原样命名（如 `walk`）；BMB 只查这些名字，不猜 ACT。若 `LookupSequence` 找不到目标序列，必须回退 idle；idle 也不存在才回到旧 Activity 层。walk/run playback rate 按当前水平速度 / `AnimationReferenceSpeeds` 参考速度缩放，避免腿部循环和实际位移打滑。某个 mob 已接入 sequence 后，客户端 `UpdateBMBVisualBones` 不得再程序化摆腿；腿归模型 sequence，骨骼覆盖只做头部/看向/吃草这类上层姿态。但单个 mob 可以在转换器 pivot 和低速 playback/cycle 问题修好前暂不接入 sequence；当前 sheep 的 `AnimationSequences` 暂时注释掉，保留持久 `limbSwingAmount`（`FrameTime` Lerp）+ 速度积分相位的程序化腿摆，当前摆幅 25 度、`LimbSwingPhaseScale=0.13`。原版羊头不会跟身体晃，locomotion 层不要做 head swing，也不要每帧锁死 head 骨；只允许吃草/preview/未来看向系统按状态接管头部。不能用 `speed > 8 then` 硬切。
